/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CustomerInfoUpdateManyWithoutCustomerLevelsInput } from "./CustomerInfoUpdateManyWithoutCustomerLevelsInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
  IsInt,
  Min,
  Max,
} from "class-validator";
import { Type } from "class-transformer";

@InputType()
class CustomerLevelUpdateInput {
  @ApiProperty({
    required: false,
    type: () => CustomerInfoUpdateManyWithoutCustomerLevelsInput,
  })
  @ValidateNested()
  @Type(() => CustomerInfoUpdateManyWithoutCustomerLevelsInput)
  @IsOptional()
  @Field(() => CustomerInfoUpdateManyWithoutCustomerLevelsInput, {
    nullable: true,
  })
  customerInfos?: CustomerInfoUpdateManyWithoutCustomerLevelsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  level?: string | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  potentialValue?: number | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  transactionHistory?: string | null;
}

export { CustomerLevelUpdateInput as CustomerLevelUpdateInput };
